...
@vars ({ @variable (
		name = PluralActionTopicMessage.OBJECT_NAME,
		type = IType.STRING,
		doc = { @doc ("Returns the concerned unity game object name") }),
		@variable (
				name = PluralActionTopicMessage.METHOD_NAME,
				type = IType.STRING,
				doc = { @doc ("Returns the unity game object method name") }),
		@variable (
				name = PluralActionTopicMessage.ATTRIBUTES_LIST,
				type = IType.MAP,
...
public class PluralActionTopicMessage extends GamaMessage {
	...
	public PluralActionTopicMessage copy(final IScope scope) throws GamaRuntimeException {
		return new PluralActionTopicMessage(scope, getSender(), getReceivers(), getObjectName(), getMethodName(),
				getAttributesList(), getContents(scope));
	}
