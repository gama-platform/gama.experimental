...
global {
    ...
    action handle_dyke_click(point click_location) {
        if (dyke_building_active) {
            if (waiting_for_first_point) {
                ...
                write "   → Building dyke...";
                // write "   Distance: " + (distance(dyke_point1, dyke_point2) with_precision 1) + "m";
                ...
                ask simulation_manager {
                    ...
                    if (success) {
                        ...
                        // write "   Length: ~" + (distance(dyke_point1, dyke_point2) with_precision 1) + "m";
                        write "   Active dykes: " + get_active_dyke_count();
                        
                        // Update global display field safely
                        if (dyke_field != nil) {
                            display_dyke_field <- copy(dyke_field);
                        }
                        write display_dyke_field where (each > 0);
                    } else {
    ...
    action report_global_status {
        ...
        if (dyke_building_active) {
            string status <- waiting_for_first_point ? "Waiting for first point" :
                            (waiting_for_second_point ? "Waiting for second point" : "Ready");
...
species simulation_manager skills: [spreading] {
    ...
    init {
        ...
        // FIXED: Create dyke field in GAMA to ensure proper coordinate system alignment
        field dyke_field_for_java <- field(elevation_map.columns, elevation_map.rows);
        
        // Initialize dyke field with same bounds as elevation map
        loop i from: 0 to: elevation_map.columns - 1 {
            loop j from: 0 to: elevation_map.rows - 1 {
        ...
        // FIXED: Use correct parameter names and order for the Java method
        do initialize_spreading_grid_with_dyke_field(
            dem_field: elevation_map,
            dyke_field: dyke_field_for_java,
        ...
        write "✓ Dyke field created with proper coordinate alignment";
    ...
    action reset_simulation {
        ...
        // FIXED: Recreate dyke field with proper coordinate system after reset
        ...
        // FIXED: Re-initialize with correct parameter names and order
        do initialize_spreading_grid_with_dyke_field(
            dem_field: elevation_map,
            dyke_field: dyke_field_for_reset,
            water_geometries: water_geometries,
            initial_water_depth: 1.5,
            flow_threshold: 0.01,
            rising_rate: 0.3,
        ...
        write "   Dyke field recreated with proper coordinate alignment";
    }
    ...
    action start_extreme_rain {
        do start_rain(0.5, 2.0); // 0.5m/step rain rate, 2.0x intensity
    ...
    action quick_status {
        ...
        write "📊 QUICK STATUS: " + sim_status + " | " + rain_status + " | " + dyke_status + 
              " | Water: " + get_active_water_count() + " cells";
    }
}
...
experiment FloodSimulationWithDykesComplete type: gui {
    ...
    // parameter "Auto-report interval (cycles)" var: auto_report_interval category: "Monitoring" min: 0 max: 100 init: 50;
    ...
    output {
        ...
        display "Flood Simulation with Rain and Dykes" type: opengl  refresh: true {
            ...
            // === EVENT HANDLERS ===
            event #mouse_down {
                ask world {do handle_dyke_click(#user_location);}
            }
            ...
            event #mouse_move {
                ...
                if (!dyke_building_active) {
                    if (display_water_field != nil) {
                        ...
                        if (water_level > 0.01) {
                            draw string("💧 " + (water_level with_precision 1) + "m") 
                                 at: #user_location + {0, 0, 10} color: #cyan font: font("Arial", 12, #bold);
                        }
                    }
                    ...
                    if (display_dyke_field != nil) {
                        ...
                        if (dyke_height > 0.01) {
                            draw string("🏗️ " + (dyke_height with_precision 1) + "m") 
                                 at: #user_location + {0, 0, 15} color: #orange font: font("Arial", 12, #bold);
                        }
                    }
                }
            }
            ...
            overlay position: {5, 5} size: {350, 140} background: #black transparency: 0.3 border: #white {
                ...
                string rain_text <- "Rain: " + (first(simulation_manager).is_rain_active() ? 
                    ("Active (" + (first(simulation_manager).get_rain_rate() with_precision 1) + "m/s)") : "None");
                ...
                string mode_text <- "Mode: " + (dyke_building_active ? 
                    (waiting_for_first_point ? "🔨 Click 1st point" : 
                     (waiting_for_second_point ? "🔨 Click 2nd point" : "🔨 Building")) : "Normal");
        ...
        monitor "🚀 Simulation Active" value: first(simulation_manager).is_simulation_active() 
               color: first(simulation_manager).is_simulation_active() ? #green : #red;
        ...
        // Rain monitors  
        monitor "🌧️ Rain Active" value: first(simulation_manager).is_rain_active() 
               color: first(simulation_manager).is_rain_active() ? #blue : #gray;
        ...
        // Dyke monitors
        monitor "🔨 Dyke Building Mode" value: dyke_building_active 
               color: dyke_building_active ? #orange : #gray;
        monitor "🏗️ Active Dyke Cells" value: first(simulation_manager).get_active_dyke_count();
        monitor "⚔️ Dyke Cells Under Attack" value: first(simulation_manager).get_surrounded_dyke_count() 
               color: first(simulation_manager).get_surrounded_dyke_count() > 0 ? #red : #green;
        monitor "🎯 Building Status" value: dyke_building_active ? 
               (waiting_for_first_point ? "First Point" : 
                (waiting_for_second_point ? "Second Point" : "Ready")) : "Inactive";
        ...
        monitor "💧 Water Coverage %" value: ((first(simulation_manager).get_active_water_count() / 
               (first(simulation_manager).grid_width * first(simulation_manager).grid_height)) * 100.0) with_precision 1;
    }
    ...
    action ask_report_status {
        ask simulation_manager { do report_status(); }
    }
    
    action ask_quick_status {
        ask simulation_manager { do quick_status(); }
    }
    
    action ask_global_status {
        ask world {do report_global_status();}
    }
    
    // Rain control actions
    action ask_start_light_rain {
        ask simulation_manager { do start_light_rain(); }
    }
    
    action ask_start_heavy_rain {
        ask simulation_manager { do start_heavy_rain(); }
    }
    
    action ask_start_extreme_rain {
        ask simulation_manager { do start_extreme_rain(); }
    }
    
    action ask_stop_rain {
        ask simulation_manager { do stop_rain_completely(); }
    }
    
    action ask_increase_rain {
        ask simulation_manager { do increase_rain(); }
    }
    
    action ask_decrease_rain {
        ask simulation_manager { do decrease_rain(); }
    }
    
    // Dyke control actions
    action ask_toggle_dyke_building {
        ask world {do toggle_dyke_building();}
    }
    
    action ask_clear_dykes {
        ask simulation_manager { do clear_dykes(); }
    }
    
    action ask_emergency_clear {
        ask simulation_manager { do emergency_dyke_clear(); }
    }
