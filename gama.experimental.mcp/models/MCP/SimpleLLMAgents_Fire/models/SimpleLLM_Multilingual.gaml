...
global {
	...
	init {
		...
		create AI {
			...
			llm_querry <- " There is a fire in my buiding. My son is on another floor. I want him to escape the building. What should I tell him ? just answer the sentence I should tell him. Nothing else";
			
			// Send the prompt to the LLM and store its answer
			mymsg <- send_to_llm(llm: chat_model, message: role + chat_memory + llm_querry );
			mymsglit <- send_to_llm(llm: chat_model, message: "respond only the translation in" + language + " of'''"+ mymsg + "''. Do not add any comment.");
...
species AI skills: [mcp_skill] {
	...
	reflex chating when: (comingmsg != nil or chat_desire) {
		...
		if comingmsg != nil {
			...
			mymsg <- send_to_llm(llm: chat_model, message: role + chat_memory + llm_querry + "[/INST]");
			mymsglit <- send_to_llm(llm: chat_model, message: "respond only the translation in" + language + " of'''"+ mymsg + ". [/INST]");
	...
	aspect default {
		...
		loop i from: 0 to: length(wrapped_lines) - 1 {
			draw wrapped_lines[i] at: location + {18, -12 + dy, 10}
				font: font("Helvetica", 20, #bold) color: #red;
			dy <- dy + 6;
		}
...
experiment Dialog type: gui {
	   parameter "Langue" var: language among: ["Français", "Vietnamese", "English", "Español", "Deutsch", "Italiano", "Русский", "العربية", "中文", "日本語", "한국어", "Tiếng Việt", "Português"] init: "Vietnamese";
 	   parameter "llmmodel" var: llmmodel among: ["llama3.2", "llama3:8b", "mistral", "gemma:2b", "phi3", "phi2", "codellama:7b", "neural-chat", "orca-mini", "tinyllama", "llava:7b"] init: "llama3.2";
